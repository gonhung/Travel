...
import HotelDetails from '../details/[id]/page';
import Card from './Card';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { deleteReservation, getUserReservations } from './services';
        ...
        <div className="mt-24 px-16 min-h-screen w-full">
            <div className="h-full w-full flex flex-wrap gap-12">
                {data?.length > 0 ? data?.map((reservation) => (
                    ...
                    mutate = {mutate}/>
                )) : <h1 className="text-center text-3xl font-bold text-slate-700">You have no reservation.</h1>}
            </div>
        </div>
    );
};
